...
class NotesApp extends React.Component {
  ...
  viewNote(id) {
    ...
    if (notePosition >= 0) {
      ...
      console.warn(
        'The note with the id ' + id + ' was not found. Please try again.'
      );
    }
  }
  ...
  saveEditedNote(note) {
    ...
    if (notePosition >= 0) {
      ...
      console.warn(
        'The note with the id ' + note.id + ' was not found. Please try again.'
      );
    }
    this.setState({
      selectedNote: note,
    ...
    return (
      <div>
        ...
        <Route
          ...
          path='/note'
          render={(routeProps) => (
            <NewNotes persistNote={this.addNote} note={this.getEmptyNote()} />
          )}
        />
      </div>
    );
    ...
    return (
      <div className='notesApp container-fluid'>
        ...
        <div className='row'>
          ...
          <div className='col-12'>{this.setMainAreaRoutes()}</div>
        </div>
      </div>
    );
  }
